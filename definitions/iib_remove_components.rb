...
define :iib_remove_components do
  ...
  testString = `sudo su - #{iibusername} -c \'mqsilist -d 0\'` 
  ...
  scanMatch.each do | match_entry |
    ...
    execute "stop mq listener" do
      user 'root'
      returns [0,13,19]
      command "sudo su - #{iibusername} -c \'endmqlsr -m #{iib_queue_manager}\'"
      ignore_failure true
    end
    ...
    execute "Stop existing IIB components" do
      user 'root'
      returns [0,13,19]
      command "sudo su - #{iibusername} -c \'mqsistop -i #{iib_node_name} \'"
      ignore_failure true
    end
    ...
    execute "Delete existing IIB components" do
      user 'root'
      returns [0,13,19]
      command "sudo su - #{iibusername} -c \'mqsideletebroker #{iib_node_name} -qw \'"
      ignore_failure true
    end
