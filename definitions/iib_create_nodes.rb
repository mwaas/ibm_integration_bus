  ...
  iib_nodes.each do | iib_node |
    ...
    if createDCW == false 
      ...
      if iib_nodes_item['node']['properties'] != nil
        ...
        # 
        # Find the listner port if it is present
        #
        if iibqmgr_port = iib_nodes_item['qmgrListenerPort'] != nil
          iibqmgr_port = iib_nodes_item['qmgrListenerPort'];
        end
      end
    else #i.e. if createDCW == true
      #
      # create a setup that is the same as the toolkit default configuration wizard  
      #
      iibnode_name = 'CL1NODE1';
      iibqmgr_name  = 'CL1QM1';
      iibserver_names = Array.new;
      iibnode_webadmin_enabled = 'true';
      iibnode_webadmin_port = 4415;
      iibnode_httplistener_port = 7080;
      iibnode_operationMode = nil;
      iibserver_names.push('CSF_IS');
      iibserver_names.push('DEMO_IS');
      iibqmgr_port   = 2414;
    end #end of if createDCW statement
    ...
    # Ensure that queue manager name is not missing
    if iibqmgr_name == nil
      raise "Queue manager name missing from the data bag item.";
    end 
  
    # Ensure that node name is not missing
    if iibnode_name == nil
    ...
    execute "Stop existing IIB node #{iibnode_name} if it exists" do
      user 'root'
      returns [0,13,19]
      command "sudo su - #{iibusername} -c \'mqsistop -i #{iibnode_name}\'"
      ignore_failure true
    end
    ...
    execute "Delete existing IIB node #{iibnode_name} if it exists" do
      user 'root'
      returns [0,13,19]
#      command "sudo su - #{iibusername} -c \'mqsideletebroker #{iibnode_name} -qw \'"
      command "sudo su - #{iibusername} -c \'mqsideletebroker #{iibnode_name} -w \'"
      ignore_failure true
    end
...
#      ignore_failure true
    ...
    if iibnode_webadmin_enabled.casecmp("true") == 0
      execute "Change IIB node #{iibnode_name} web admin port to #{iibnode_webadmin_port}" do
        user 'root'
        returns [0]
        command "sudo su - #{iibusername} -c \'mqsichangeproperties #{iibnode_name} -b webadmin -o HTTPConnector -n port -v #{iibnode_webadmin_port}\'"
      end
    else
      execute "Disable IIB node #{iibnode_name}'s web admin" do
        user 'root'
        returns [0]
        command "sudo su - #{iibusername} -c \'mqsichangeproperties #{iibnode_name} -b webadmin -o server -n enabled -v  #{iibnode_webadmin_enabled}\'"
      end
    end
    ...
    if iibnode_operationMode != nil
      execute "Set operation mode on IIB node #{iibnode_name} to #{iibnode_operationMode}" do
        user 'root'
        returns [0]
        command "sudo su - #{iibusername} -c \'mqsimode #{iibnode_name} -o #{iibnode_operationMode}\'"
      end
    end
...
#      ignore_failure true
    ...
    iibserver_names.each do | iibserver_name | 
      execute "Create IIB node server #{iibserver_name}" do
        user 'root'
        returns [0]
        command "sudo su - #{iibusername} -c \'mqsicreateexecutiongroup #{iibnode_name} -e #{iibserver_name} -w 300\'"
      end
    end

  end #end of iib_nodes.each
end
